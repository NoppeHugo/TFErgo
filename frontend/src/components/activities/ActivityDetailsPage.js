...
const ActivityDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const mapped = act.materials.map(m => ({ value: m.material.id, label: m.material.name }));
  ...
  const goToNext = () => setCurrentImage((currentImage + 1) % existingImages.length);
  const goToPrev = () => setCurrentImage((currentImage - 1 + existingImages.length) % existingImages.length);
    ...
    >
      <div className="max-w-5xl mx-auto relative">
        ...
        {!editing ? (
              ...
              <div className="relative mt-8 w-full max-w-5xl mx-auto overflow-hidden">
                <div className="relative w-full h-[400px] flex items-center justify-center">
                  {existingImages.length > 1 && (
                    <button onClick={goToPrev} className="absolute left-0 top-1/2 -translate-y-1/2 bg-white/70 hover:bg-white p-2 text-gray-700 z-10 rounded-l">
                  ...
                  <div className="w-full overflow-hidden">
                    <div
                      ...
                      style={{
                        transform: `translateX(-${(100 / existingImages.length) * currentImage}%)`,
                        width: `${existingImages.length * 100}%`,
                      }}
                    >
                      {existingImages.map((file, index) => (
                        <div key={file.id} className={`${existingImages.length === 1 ? 'w-full' : 'w-[33.3333%]'} flex-shrink-0 flex items-center justify-center`}>
                          <img src={file.fileUrl} alt={`Image ${index + 1}`} className="max-h-[400px] object-contain" />
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {existingImages.length > 1 && (
                    <button onClick={goToNext} className="absolute right-0 top-1/2 -translate-y-1/2 bg-white/70 hover:bg-white p-2 text-gray-700 z-10 rounded-r">
          ...
          <form onSubmit={handleUpdate} className="bg-white p-6 rounded border shadow space-y-4 animate-fade-in mt-6">
            ...
            <div>
              ...
              <Select options={goals.map((g) => ({ value: g.id, label: g.name }))} isMulti value={selectedGoals} onChange={setSelectedGoals} className="text-sm" />
            </div>
            ...
            <div>
              ...
              <input type="file" multiple accept="image/*" onChange={handleAddImages} className="text-sm" />
              {newImages.length > 0 && (
