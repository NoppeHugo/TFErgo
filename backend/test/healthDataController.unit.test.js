...
describe('Health Data Controller', () => {
  ...
  it('GET /healthData/:id retourne les données santé du patient', async () => {
    ...
    if (res.statusCode === 200) {
      ...
      expect(res.body).toHaveProperty('medicalDiagnosis');
      expect(res.body).toHaveProperty('medicalHistory');
      expect(res.body).toHaveProperty('healthChronicle');
    }
  });
  ...
  it('GET /healthData/:id retourne 404 si le patient n\'existe pas', async () => {
    const res = await request(app)
